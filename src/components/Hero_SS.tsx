...
const Hero = () => {
  ...
  // Reset cycle completed state when playing is toggled
  useEffect(() => {
    if (isPlaying) {
  ...
  const startMediaAdvance = useCallback(() => {
    ...
    mediaIntervalRef.current = setInterval(() => {
      setActiveMediaIndex((prev) => {
        ...
        // Check if we're about to return to the first slide (completed a full cycle)
        if (next === 0 && prev === mediaSlides.length - 1) {
          setCycleCompleted(true);
          setIsPlaying(false); // Pause the slideshow
        }
  ...
  const changeSlide = useCallback((newIndex: number) => {
    ...
    // Reset cycle completed state when manually changing slides
    setCycleCompleted(false);
    
    // If not playing, we should still allow manual navigation
    if (!isPlaying) {
      // Just update the slides, don't start intervals
      return;
    }
    ...
    startMediaAdvance();
    startTextAdvance();
  }, [startMediaAdvance, startTextAdvance, isPlaying]);
  ...
  const nextSlide = useCallback(() => {
    ...
    // Reset cycle completed state
    setCycleCompleted(false);
    
    // Only reset timers if playing
    if (isPlaying) {
      startMediaAdvance();
      startTextAdvance();
    }
  }, [mediaSlides.length, startMediaAdvance, startTextAdvance, isPlaying]);
  ...
  const prevSlide = useCallback(() => {
    setActiveMediaIndex((prev) => {
      ...
      // Reset cycle completed state when manually navigating
      setCycleCompleted(false);
  ...
  const togglePlayPause = useCallback(() => {
    setIsPlaying(prev => {
      ...
      // If we're unpausing and the cycle was completed, reset to first slide
      if (newPlayingState && cycleCompleted) {
  ...
  return (
    <section id="home" className="relative h-screen overflow-hidden">
        ...
        <div className="text-center max-w-4xl px-4">
          <AnimatePresence mode="wait">
            <motion.h1
              ...
              animate="animate"
              exit="exit"
              className="text-white text-4xl md:text-4xl lg:text-4xl font-medium drop-shadow-2xl"
      ...
      {/* Play/Pause Button - 25% larger */}
      <button
        onClick={togglePlayPause}
        className="absolute top-6 right-6 z-20 bg-black/50 hover:bg-black/70 text-white rounded-full p-4 transition-all duration-300 backdrop-blur-sm border border-white/20"
